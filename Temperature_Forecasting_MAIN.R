...
library(tidyverse)    # (For working with multiple packages)
library(inspectdf)    # (Inspection, comparsion and visualization of Data Frames)
library(dplyr)        # (For different kinds of data manipulation techniques)
library(data.table)   # (Aggregation of data frames, data mining)
library(lubridate)    # (For working with dates and times)
library(timetk)       # (Easy visualization, wrangling, and feature engineering of time series data for forecasting and machine learning prediction)
library(skimr)        # (Compact and flexible summaries of data)
library(highcharter)  # (For creating dynamic visuals)
library(h2o)          # (Machine Learning Initializer)
library(rsample)      # (General Resampling Infrastructure)
library(forecast)     # (For time series forecasting)
library(tidymodels)   # (For time series modelling)
library(modeltime)    # (Forecasting framework for time series)
